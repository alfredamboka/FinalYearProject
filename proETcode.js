var geometry2 = ee.FeatureCollection('projects/project5yr/assets/Croplands');
var geometry = geometry2.geometry();
Map.centerObject(geometry2);
Map.addLayer(geometry2, {color: 'red'}, 'BuraScheme');

//Variables Needed Month Number and Year Number
//Month number
var m = 9;   // can be 7, 4, 6, 3, 6 - for Short season or 12, 12,9,9,10 - for Long season(LS) for maturity in Bura Irriation
//Year number
var year = 2021; //can be replaced with: 2018, 2019,2020,2021, 2022
...
// Load morning (Terra) MODIS data.
  var morning = ee.ImageCollection('MODIS/061/MOD09GA')
...
// Calculate median
var s2m = median.select('B2', 'B3', 'B4', 'B5', 'B6', 'B7', 'B8', 'B11', 'B12');

var albedo_img = s2m.expression(
...
// Calculate NDVI
var ndvi = s2m.normalizedDifference(['B8','B4']);
...
var tempRna = tempS_na.subtract(tempL_n);

// Total radiation (w/ha/month)
var rna = tempRna.multiply(ee.Number(monthday)).divide(ee.Number(864));
...
var median1 = LST.median();

// Calculate median
var LSTm = median1.select('LST_Day_1km').multiply(0.02);
...
var rah_1 = Fri_200.multiply(von);
...
// The result is a Dictionary.  Print it.
var dem_coldmean = dem_coldmeandict.get('LST_Day_1km');
...
// The result is a Dictionary.  Print it.
var dem_coldstd = dem_coldmeandict1.get('LST_Day_1km');
...
// The result is a Dictionary.  Print it.
var lst_dt_cold = dt_coldlst.get('LST_Day_1km');
...
// The result is a Dictionary.  Print it.
var dem_hotmean = dem_hotmeandict.get('LST_Day_1km');
...
// The result is a Dictionary.  Print it.
var dem_hotstd = dem_hotmeandict1.get('LST_Day_1km');
...
// The result is a Dictionary.  Print it.
var lst_dt_hot = dt_hotlst.get('LST_Day_1km');
...
// The result is a Dictionary.  Print it.
var meanhw1 = dict_w1.get('LST_Day_1km');
...
// The result is a Dictionary.  Print it.
var meandtw_1 = dict_dtw.get('LST_Day_1km');
...
Export.image.toDrive({
  image: albedo_img,
  description: 'albedo_'+ 'LS_'+ year,
  scale: 10,
  maxPixels: 3784216672400,
  folder: 'GEEMainProject',
  region: geometry
});
